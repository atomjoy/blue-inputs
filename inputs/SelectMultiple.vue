...
const props = defineProps({
	name: { type: String, default: 'multiple[]' },
	...
	<div class="form-input-wrapper">
		<div class="form-input-event" @click.stop="toggle('div', $event)">
				...
				<div class="form-select-options-wrapper">
					<div class="form-select-option" @click.stop="update(option)" v-for="option in filtered_options">
						<div class="form-select-option-icon">
							<!-- <i v-if="model == option">✔️</i> -->
							<IconCheckmark v-if="model.includes(option)" />
						</div>
			...
			<select multiple v-model="model" :name="props.name" style="display: none">
				<option v-for="o in props.options" :value="o" :key="o" :selected="model.includes(o)">{{ o }}</option>
			</select>
		</div>
	</div>
</template>
