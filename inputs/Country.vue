...
function searchName(str) {
	let item = country.filter((o) => {
		return o.code.toLowerCase().startsWith(str.toLowerCase());
	});
	return item[0].name ?? 'Invalid code';
}

function code(str) {
	let item = country.filter((o) => {
		return o.code.toLowerCase() == str.toLowerCase();
	});
	return item[0].code ?? 'Invalid code';
}
		...
		<div class="form-input-event" @click.stop="toggle('div', $event)">
			<div class="form-select">
				<div class="form-select-selected">
					<img class="form-select-option-flag-selected" :src="'/flags/' + code(model) + '.webp'" onerror="this.remove()" />
					{{ searchName(model) ?? props.placeholder }}
				</div>
			</div>
			...
			<div class="form-select-options">
				<input v-model="search" type="text" placeholder="Search" class="form-select-options-search" @click.stop="" @keyup="filterOptions" />
